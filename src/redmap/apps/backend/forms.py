...
from redmap.apps.redmapdb.models import Sighting, SpeciesAllocation, Region, Species,\
    Organisation, Person, AdministratorAllocation
from redmap.apps.backend.models import ValidationMessageTemplate, SightingValidationRule,\
...
class VerifyStep(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set the initial sighting importance characteristics from the species
        instance = kwargs.get('instance', None)
        if instance and instance.species:
            kwargs['initial'].update({
                'sighting_interests': instance.species.species_interests.all(),
            })

        super(VerifyStep, self).__init__(*args, **kwargs)

        # Append a radiogroup for each ConditionSection. The scientist must
        # check each of these options off to approve/reject the sighting.
        ...
        # The 'Photo identifies the species' option should *not* be required if
        # there is no photo
    ...
    class Meta:
        ...
        fields = ['species', 'other_species', 'sex', 'size_method', 'size', 'weight', 'count',
            ...
            'activity', 'notes', 'photo_caption', 'sighting_interests']
        widgets = {
            'sighting_interests': forms.widgets.CheckboxSelectMultiple(),
        }
...
class UserAddForm(ModelForm):
    ...
    def save(self, *args, **kwargs):
        ...
        # TODO: translate boolean into integer
...
class EditSightingForm(ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'other_species', 'sighting_interests'
        ]
        widgets = {
            'sighting_interests': forms.widgets.CheckboxSelectMultiple(),
        }
...
class SightingsListSearchForm(FilterSet):
    ...
    class Meta:
        ...
        order_by = [
            # Unfortunately, we have to duplicate the order params here to
    ...
    def __init__(self, data, *args, **kwargs):
        ...
        # The above queryset changes will not be reflected in the form, as the
        # form is cached. We need to delete the cached form, and the cached
        # field values to get a completely new form
        delattr(self, '_form')
        for filter_ in self.filters.values():
            delattr(filter_, '_field')


class SightingsExportForm(forms.Form):
    accept_privacy_warning = forms.BooleanField(required=True, initial=False)
    region = forms.ModelChoiceField(queryset=Region.objects.none())

    def __init__(self, *args, **kwargs):
        self.user = kwargs.pop('user', None)
        super(SightingsExportForm, self).__init__(*args, **kwargs)

        if self.user.is_superuser:
            self.fields['region'].empty_label = "-- Download all --"
            self.fields['region'].required = False
        if self.user:
            self.fields['region'].queryset = Region.objects.filter(administratorallocation__in=self.user.administratorallocation_set.all())
