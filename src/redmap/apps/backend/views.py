from redmap.apps.backend.forms import UserAddForm, OrganisationAddForm, AddScientistForm, \
...
from redmap.apps.backend.models import ValidationMessageTemplate, ValidationResponse, \
...
from redmap.apps.redmapdb.models import Sighting, SpeciesAllocation, SightingTracking, \
...
from redmap.apps.backend.forms import SightingsExportForm
from redmap.common.views import BaseCsvExportMixin
...
def is_sighting_valid(wizard):
    ...
    # Fetch the conditions which the validator selected
    conditions = wizard.get_cleaned_data_for_step('0').get('conditions')

    # Fetch details about sighting
    ...
    if rule is not None:
        ...
        if not assessment:
            # The assessment was blank, populate with a default value
        ...
        # TODO: Should have a default case for when no matching rule is found
...
class ScientistVerifyWizard(AuthMixin, SessionWizardView):
    ...
    def done(self, form_list, *args, **kwargs):
        ...
        # TODO: form=form_list looks like a bug
        context = self.get_context_data(form=form_list)

        # Check if sighting is valid
        result = is_sighting_valid(self)

        # Update the sighting data with any changes provided by the scientist
        sighting = context['sighting']
        for form in form_list:
            for field, value in form.cleaned_data.iteritems():
                setattr(sighting, field, value)

        # Save changes made by the scientist to the databaes
        sighting.save()

        # Check we have a valid sighting tracking object
        ...
        # Check that the validating user owns that tracker (sometimes regional
        # admins like to validate other peoples sightings)
        if not sighting.tracker.person == self.request.user:
            sighting.reassign(sighting, self.request.user,
                'Reassigned to %s in order to validate'
                % self.request.user.profile)

        # Track responses for future reference
        responses = self.get_responses()
        ValidationResponse.objects.record_responses(sighting, responses)

        # Update sighting tracking object
        ...
        # TODO: Investigate if render_to_string will work here, where the
        #       message is also a variable
        # TODO: List of these are currently hardcoded into the
        #       'add_email_template.html' file. Would be cleaner to loop
        ...
        # get the current transaction and flush it
        transaction.commit()

        # Post sighting to the sighter's facebook wall if they have a linked account
    ...
    def get_context_data(self, form, **kwargs):
        ...
        # Fetch the sighting we're validating
        sighting = self.instance_dict['0']  # TODO: better as an arg really

        # Add the sighting to the context for templates
        context['sighting'] = sighting

        # Report form has some additional context variables
        if self.steps.current == "1":

            # Find the validation rules which match our report
            matching_rules = SightingValidationRule.objects.filter(
                valid_sighting=is_sighting_valid(self).get('valid'))

            # Add the matching templates to the context
            context['validation_templates'] = \
                ValidationMessageTemplate.objects.filter(
                    sightingvalidationrule__in=matching_rules)

        # The map requires accuracies.
        # TODO: would this be a better context_processor since maps are used
        #       all over the place
        ...
        for a in accuracies:
            ...
            try:
                ...
                # FIXME: This is a bad assumption (ie. that slicing off the
                #        last character will fix this type issue)
                #        See Ticket #1609
                a.code = a.code[0:-1]
        ...
        # Find the most recent sighting tracking object
        # TODO: Is this required?  Could make sense for revalidation.
    ...
    def get_form_kwargs(self, step):
        ...
        # Report form needs some keyword args to create
        if step == "1":
...
class manage_experts(AuthMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['species_list'] = [
            (s.id, u'{0} ( {1} )'.format(s.species_name, s.common_name), ) \
...
def AddScientist(request):
    ...
    if request.POST and form.is_valid():
        ...
        # grab the validated objects from our form
        user_id = form.cleaned_data.get('username')
        user = User.objects.get(pk=user_id)
        organisation = form.cleaned_data.get('organisation')

        # add user to the Scientists group
        group = Group.objects.get(name="Scientists")
        group.user_set.add(user)

        # set the user's organisation on their profile
...
# TODO: should be for admin only (not regional admin)
@login_required
...
def add_validation_rule(request,
    ...
    # NOTE This is a dirty hack, but I can not think of a better way to do it.
    # The constraints are:
    #
    #  * There must be a `RuleConditionTest` form for each
    #    `SightingValidationCondition`.
    #  * We can not create a bunch of `RuleConditionTest` instances and assign
    #    them to `rule.condition_tests,` as `rule` has not been saved yet.
    #  * Using the `initial` argument whenc creating the formset shows at most
    #    `extra` fields. `extra` is set by the factory, not when creating the
    #    the formset instance.
    #
    # The only solution left that I could see was creating a new formset with a
    # dynamically calculated `extra` parameter.
...
def edit_validation_rule(request, pk,
    ...
    # Just running this is enough, as the method creates and saves any
    # missing RuleConditionTests for this SightingValidationRule
...
class MemberIndex(AuthMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if filters.get('activated', None) == "False":
            # Look through the registration module for users which are
            # unactivated
...
class SightingsExportView(AuthMixin, BaseCsvExportMixin, FormView):

    template_name = "backend/sightings_export.html"

    or_required_permissions = [
        'redmapdb.can_export_all_sightings',
        'redmapdb.can_export_sightings',
    ]

    form_class = SightingsExportForm
    attachment_name = "redmap-sightings-export"

    @method_decorator(login_required)
    def dispatch(self, request, *args, **kwargs):
        print request.user.user_permissions.all()
        user_permissions = map(lambda p: u"{0}.{1}".format(p.content_type.app_label, p.codename), request.user.user_permissions.all())
        allowed = any([unicode(p) for p in self.or_required_permissions for p in user_permissions] + [request.user.is_superuser])

        self.can_export_sightings = 'redmapdb.can_export_sightings' in user_permissions
        self.can_export_all_sightings = 'redmapdb.can_export_all_sightings' in user_permissions

        if not allowed:
            raise PermissionDenied
        return super(SightingsExportView, self).dispatch(request, *args, **kwargs)

    def get_form_kwargs(self):
        kwargs = super(SightingsExportView, self).get_form_kwargs()
        kwargs['user'] = self.request.user
        return kwargs

    def get_dev_result(self, region=None):
        return [
            ['column1', 'column2'],
            ['data1', 'data2'],
        ]

    def get_mssql_result(self, region=None):

        if (self.request.user.is_superuser or self.can_export_all_sightings) and region is None:
            sightings = Sighting.objects.function_get_sighting_export()
        elif (self.request.user.is_superuser or self.can_export_all_sightings or self.can_export_sightings) and region:
            sightings = Sighting.objects.function_get_sighting_export(region)
        else:
            sightings = []
        return sightings
    ...
    def form_valid(self, form):
        ...
        region = cd.get('region', None)

        if not settings.REDMAP_MODELS_USE_MSSQL:
            data = self.get_dev_result(region)
        else:
            data = self.get_mssql_result(region)

        return self.make_csv_response(self.request, data)
...
def SightingEdit(request, pk=None, template_name="backend/sighting_edit.html"):
    ...
    for a in accuracies:
        try:
            ...
            # FIXME: This is a bad assumption (ie. that slicing off the last
            #        character will fix this type issue)
            #        See Ticket #1609
            a.code = a.code[0:-1]
        accuracy_dict.update({a.id: a.code})
...
def resend_member_activation(request, user_id):
    ...
    if profile.is_pending_activation and request.POST and confirming:
        # collect the site and registration objects
        ...
        # success message and redirect back to members index
