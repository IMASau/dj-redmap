...
from redmap.apps.frontend.forms import (
...
# TODO: move into a helper function common.utils?
def get_exif(fn):
    '''
        ...
        if info is not None:
            for tag, value in info.items():
                ...
                if decoded == 'GPSInfo':
                    ...
                    # If we have GPS EXIF data, be sure to decode it
                    for t, v in value.items():
                        d = GPSTAGS.get(t, t)
                        new_value[d] = v

                    if 'GPSLatitude' not in new_value.keys():
                        return None

                    # Convert GPS data into latitude/longitude
                    try:
                        latitude_dms = [(tup[0] / tup[1]) for tup in new_value['GPSLatitude']]
                    ...
                    try:
                        longitude_dms = [(tup[0] / tup[1]) for tup in new_value['GPSLongitude']]
                    except ZeroDivisionError:
                        return None
...
class AddWizard(UploadHandlerMixin, SessionWizardView):
    ...
    def done(self, form_list, *args, **kwargs):
        ...
        # TODO: no exception management here
    ...
    def get_context_data(self, form, **kwargs):
                ...
                try:
                    ...
                    if exif:
                        ...
                        if date_string:
                            ...
                            # TODO WTF this should be datetime.datetime, not datetime
                            # For some reason, `import datetime` some how becomes the
                            # `datetime.datetime` class. If this breaks in the future,
                            # check to see that `datetime` is not `datetime.datetime`
                            # anymore.
                            try:
        ...
        if int(self.storage.current_step) == 3:
            ...
            try:
                context.update({'survey': Survey.objects.get(slug="add_step_4")})
...
def region_landing_page(request, region_slug):
    ...
    context = {
        ...
        'recent_news': TaggedItem.objects.get_intersection_by_model(
...
class SightingDetailView(DetailView, RedmapContextMixin):
    ...
    def get_object(self, queryset=None):
        ...
        # You can always see your own sightings
        if sighting.user == self.request.user:
            return sighting

        # Else it needs to be published
    ...
    def get_context_data(self, **kwargs):
        ...
        # Find recent sightings of this species / in this area
        ...
        # If known species, get the category
        try:
...
"""
TODO: Deprecated, remove next time
def render_cluster(request, count):
    ...
    # Single clusters should be 'dots' - see #188
    if count == '1':
        count = ''
    ...
    def get_queryset(self):
        ...
        if region_slug:
            sightings = Sighting.objects.get_public_photo().\
                filter(region__slug=region_slug)
        else:
            sightings = Sighting.objects.get_public_photo()
