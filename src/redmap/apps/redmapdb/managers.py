...
,CAST({longitude} as numeric(18,9)))""".format(latitude=latitude, longitude=longitude)
    ...
    def function_get_sighting_export(self, region=None):
        # header / column names
        header_columns_call = """SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.Columns where TABLE_NAME = 'RM_SIGHTING_ADMIN_DOWNLOAD_VIEW'"""
        result, cursor = self.execute_call(header_columns_call)
        header_columns = [row[0] for row in result.fetchall()]

        # rows
        call = """SELECT * FROM [dbo].[RM_SIGHTING_ADMIN_DOWNLOAD_VIEW]"""
        if region is not None:
            call = """SELECT * FROM [dbo].[RM_SIGHTING_ADMIN_DOWNLOAD_VIEW] WHERE RM_REGION_ID = {0}""".format(region.pk)

        result, cursor = self.execute_call(call)
        return [header_columns] + result.fetchall()
