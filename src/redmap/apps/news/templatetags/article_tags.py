...
from redmap.apps.redmapdb.models import Person, Region
...
def only_tags(maybe_tag):
    ...
    if isinstance(maybe_tag, Tag):
        ...
        # Find tags based upon a tag string
        # This could be a comma-separated set of tag names.
        ...
        # This might be a list, queryset or other iterable containing tags
        try:
...
def articles_with_tags(*args):
    ...
    """
    # Remove duplicates by using set. Duplicates screw things up.
...
def news_with_tags(*args):
    ...
    tagged_entries = TaggedItem.objects.get_intersection_by_model(articles, list(tags))

    if not tagged_entries.exists():
        tagged_entries = articles
...
def region_tags(tags):
    ...
    # Find all the passed tags that are also region tags
